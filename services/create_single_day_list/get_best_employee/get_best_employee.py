...
def get_best_employee(available_employees, assigned_employees, current_position_code3, required_count, total_work_time, leader_count):
    """
    ...
    if skills_index == None:
        ...
        best_employee = select_best_employee(suitable_employees_list, total_work_time, current_position_code3, skills_index, positions_details)
        logging.debug(f'fc: get_best_employee, skills index is None: {best_employee}')

    # 現在割り当てを行っているポジションが、割り当ての条件のあるポジションの場合。
    else:
        matching_skills_employees_list = create_matching_skills_employees_list(suitable_employees_list, current_position_code3, skills_index, leader_count)
        best_employee = select_best_employee(matching_skills_employees_list, total_work_time, current_position_code3, skills_index, positions_details)
